...
// Інжектуємо стилі напряму — без окремого CSS файлу
const _style = document.createElement('style');
_style.textContent = '.circular-gallery{width:100%;height:100%;overflow:hidden;cursor:grab}.circular-gallery:active{cursor:grabbing}';
if (!document.head.querySelector('[data-cg]')) {
    _style.setAttribute('data-cg', '1');
...
class App {
    constructor(container, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px DM Sans', scrollSpeed = 2, scrollEase = 0.05, onItemClick } = {}) {
    ...
    onTouchMove(e) {
        ...
        if (Math.abs(x - this.start) > 6) this.isDragging = true;
    ...
    onTouchUp(e) {
        ...
        if (!this.isDragging && this.onItemClick && this.medias?.length) {
            const clientX = e.changedTouches ? e.changedTouches[0].clientX : e.clientX;
            const rect    = this.gl.canvas.getBoundingClientRect();

            // Проектуємо позицію кожної картки з world → screen пікселі
            // worldX → NDC: worldX / (viewport.width/2)
            // NDC → screen: (ndc + 1) / 2 * rect.width + rect.left
            const vw2 = this.viewport.width / 2;
            ...
            this.medias.forEach((media, i) => {
                ...
                const ndc     = worldX / vw2;
                const screenX = ((ndc + 1) / 2) * rect.width + rect.left;
                const halfPx  = (media.plane.scale.x / this.viewport.width) * rect.width / 2;

                // Чи потрапив клік у ширину цієї картки
                if (clientX >= screenX - halfPx && clientX <= screenX + halfPx) {
            ...
            if (bestIdx !== null) {
                const half    = this.mediasImages.length / 2;
                const origIdx = bestIdx % half;
                console.log('[Gallery] hit:', bestIdx, '→', origIdx, this.mediasImages[origIdx]?.text);
                this.onItemClick(this.mediasImages[origIdx]);
            }
        }
        this.isDragging = false;
    }
...
export default function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 28px DM Sans', scrollSpeed = 2, scrollEase = 0.05, onItemClick }) {
    ...
    // Клік через React — надійніше ніж window events
    const handleClick = (e) => {
        if (!appRef.current || !onItemClick) return;
        const app    = appRef.current;
        const rect   = containerRef.current.getBoundingClientRect();
        const clientX = e.clientX;

        // Конвертуємо X у viewport координати
        const normX   = ((clientX - rect.left) / rect.width) * 2 - 1;
        const clickVX = normX * (app.viewport.width / 2);

        // Знаходимо найближчу по X картку
        let bestIdx  = -1;
        let bestDist = Infinity;

        app.medias.forEach((media, i) => {
            const dist = Math.abs(media.plane.position.x - clickVX);
            if (dist < bestDist) { bestDist = dist; bestIdx = i; }
        });

        // Клік дійсний якщо попали у межі картки
        if (bestIdx >= 0 && app.medias[bestIdx]) {
            const hw = app.medias[bestIdx].plane.scale.x / 2;
            if (bestDist <= hw) {
                const half    = app.mediasImages.length / 2;
                const origIdx = bestIdx % half;
                const item    = app.mediasImages[origIdx];
                console.log('[Gallery] click → index:', origIdx, 'item:', item?.text);
                onItemClick(item);
            }
        }
    };
